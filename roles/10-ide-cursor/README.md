...
3. **Проверка файла:** убеждается, что скачанный файл не пустой и валидный `.deb`.
4. **Установка:** выполняет `apt-get install` для локального файла.
5. **Обработка ошибок:** выводит предупреждение при недоступности сервера.
