...
**Экран «Oh no! Something has gone wrong» при входе по RDP** — сессия Cinnamon падает, т.к. Xsession в RDP не поднимает окружение корректно. Роль создаёт у пользователя `~/.xsession` с прямым запуском `cinnamon-session`. Если ошибка уже есть на машине, создайте вручную (под пользователем, который входит по RDP) — см. блок ниже с `dbus-run-session`.

**По RDP видны только обои, нет меню и значков на рабочем столе** — сессия должна запускаться через D-Bus. В `~/.xsession` нужен запуск `dbus-run-session -- cinnamon-session`. Обновите файл вручную:
